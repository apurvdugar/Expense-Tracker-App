...
const Signup = () => {
    const API = import.meta.env.REACT_APP_API_BASE_URL || "http://localhost:8000";
    ...
    const handleSignup = async (e) => {
        ...
        setError("");
        try {
          // Make sure this URL matches your actual backend signup endpoint
          const res = await axios.post(`${API}/api/auth/signup`, form);
          // Optionally save token, set user context, etc.
  ...
  return (
    <div className="relative min-h-screen bg-linear-to-tr from-white via-slate-50 to-green-50 flex flex-col overflow-x-hidden">
      ...
      <main className="flex-1 flex items-start justify-center z-10">
        <form
          className="relative max-w-md w-full mt-3 bg-white/80 backdrop-blur-xl border border-slate-100 shadow-2xl rounded-2xl p-8 flex flex-col items-stretch gap-6 glass-effect"
