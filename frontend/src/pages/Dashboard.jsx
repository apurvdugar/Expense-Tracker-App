  ...
  return (
    <div className="relative min-h-screen bg-linear-to-tr from-white via-slate-50 to-green-50 overflow-x-hidden pt-24">
      ...
      <div className="container mx-auto max-w-7xl py-10 px-4">
        ...
        <h1 className="text-3xl font-extrabold text-green-700 tracking-tight mb-8">Financial Overview</h1>
            ...
            {recentExpenses.length === 0 ? (
              ...
              <div className="overflow-auto">
                <table className="w-full text-left">
                  <thead>
                    <tr>
                      <th className="font-semibold text-gray-700 pb-2 text-sm">Name</th>
                      <th className="font-semibold text-gray-700 pb-2 text-sm">Category</th>
                      <th className="font-semibold text-gray-700 pb-2 text-sm">Amount</th>
                      <th className="font-semibold text-gray-700 pb-2 text-sm">Date</th>
                    </tr>
                  </thead>
                  <tbody>
                    {recentExpenses.map((e, i) => (
                      <tr key={e._id || i} className="border-t text-sm">
                        <td className="py-2">{e.description}</td>
                        <td className="py-2 capitalize">{e.category}</td>
                        <td className="py-2 font-bold text-green-600">₹{e.amount}</td>
                        <td className="py-2 text-gray-600">{new Date(e.createdAt).toLocaleDateString()}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
